...
private struct LegendAgentClientsView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $isClientCreationPresented, onDismiss: {
            ...
            LegendAgentClientCreationSheet(store: store) {
...
private struct LegendAgentClientCreationSheet: View {
    @ObservedObject private var store: MobileAgentWorkspaceStore
    let onCreated: () -> Void

    @Environment(\.dismiss) private var dismiss
    @State private var form: MobileClientCreationForm?
    ...
    @State private var dateSelection: ClientCreationDateSelection?
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                LegendNextCanvas()
                if isLoading {
                    LegendScreenSkeleton(accessibilityMessage: "Loading client intake") {
        ...
        .task { await loadForm() }
        .sheet(item: $dateSelection) { selection in
            NavigationStack {
                DatePicker(
                    selection.field.label,
                    selection: dateBinding(for: selection.field.key),
                    displayedComponents: .date)
                    .datePickerStyle(.graphical)
                    .padding(LegendNextSpacing.md)
                    .navigationTitle(selection.field.label)
                    .navigationBarTitleDisplayMode(.inline)
                    .toolbar {
                        ToolbarItem(placement: .cancellationAction) {
                            Button("Clear") {
                                updateValue("", for: selection.field.key)
                                dateSelection = nil
                            }
                        }
                        ToolbarItem(placement: .confirmationAction) {
                            Button("Done") { dateSelection = nil }
                        }
                    }
            }
            .tint(LegendNextColor.gold)
        }
        .alert("Client could not be created", isPresented: submissionErrorPresented) {
            Button("OK", role: .cancel) { submissionError = nil }
        } message: {
            Text(submissionError ?? "Review the form and try again.")
        }
    }

    private func formContent(_ form: MobileClientCreationForm) -> some View {
        LegendScrollView {
            VStack(alignment: .leading, spacing: LegendNextSpacing.md) {
                ...
                if !fieldErrors.isEmpty {
                    LegendNextSurface(style: .elevated) {
                        Label(
                            "Review the highlighted fields before creating this record.",
                ...
                let actions = form.actions.filter { $0.field != nil && $0.value == "today" }
                if !actions.isEmpty {
                    LazyVGrid(
                        columns: [GridItem(.adaptive(minimum: 148), spacing: LegendNextSpacing.sm)],
                        alignment: .leading,
                        spacing: LegendNextSpacing.sm) {
                        ForEach(actions) { action in
                            Button(action.label) { perform(action) }
                                .buttonStyle(LegendNextButtonStyle(kind: .ghost))
                        }
                    }
                }
                ...
                Button {
                    ...
                    Label(
                        isSubmitting ? "Creating…" : submitTitle,
                        systemImage: isSubmitting ? "hourglass" : "person.badge.plus")
                }
                .buttonStyle(LegendNextButtonStyle(kind: .gold))
    ...
    private func sectionContent(_ section: MobileClientCreationFormSection) -> some View {
        let fields = section.fields.filter(isVisible)
        return LegendNextSurface(style: .elevated) {
            VStack(alignment: .leading, spacing: LegendNextSpacing.md) {
                ...
                ForEach(fields.filter { $0.inputKind == "choice" }) { field in
                    choiceField(field)
                }

                let compactFields = fields.filter {
                    $0.inputKind != "choice" &&
                    $0.inputKind != "multiline" &&
                    $0.inputKind != "hidden"
                }
                if !compactFields.isEmpty {
                    LazyVGrid(
                        ...
                        alignment: .leading,
                        spacing: LegendNextSpacing.sm) {
                        ForEach(compactFields) { field in
                            textField(field)
                        }
                    }
                }

                ForEach(fields.filter { $0.inputKind == "multiline" }) { field in
    ...
    private func choiceField(_ field: MobileClientCreationFormField) -> some View {
        VStack(alignment: .leading, spacing: LegendNextSpacing.tiny) {
            fieldLabel(field)
            if field.options.count <= 3 {
                LazyVGrid(
                    columns: [GridItem(.adaptive(minimum: 118), spacing: LegendNextSpacing.xs)],
                    alignment: .leading,
                    spacing: LegendNextSpacing.xs) {
                    ForEach(field.options) { option in
                        Button {
                            updateValue(option.value, for: field.key)
                        } label: {
                            Text(option.label).lineLimit(1)
                        }
                        .buttonStyle(LegendNextButtonStyle(
                            kind: values[field.key] == option.value ? .gold : .ghost,
                            controlHeight: 42))
                    }
                }
            } else {
                Menu {
                    Picker(field.label, selection: valueBinding(for: field.key)) {
                        ForEach(field.options) { option in
    ...
    @ViewBuilder
    private func textField(_ field: MobileClientCreationFormField) -> some View {
        VStack(alignment: .leading, spacing: LegendNextSpacing.tiny) {
            fieldLabel(field)
            if field.inputKind == "date" {
                Button {
                    dateSelection = ClientCreationDateSelection(field: field)
                } label: {
                    HStack(spacing: LegendNextSpacing.xs) {
                        Image(systemName: "calendar")
                            .foregroundStyle(LegendNextColor.gold)
                        Text(dateLabel(for: field))
                            .foregroundStyle(values[field.key]?.isEmpty == false
                                ? LegendNextColor.textPrimary
                                : LegendNextColor.textSecondary)
                        Spacer(minLength: 0)
                    }
                ...
                LegendNextInsetSurface {
                    TextField(field.placeholder ?? field.label, text: valueBinding(for: field.key))
                        .font(LegendNextTypography.body)
                        .foregroundStyle(LegendNextColor.textPrimary)
                        .keyboardType(keyboardType(for: field))
                        .textContentType(contentType(for: field))
                        .textInputAutocapitalization(autocapitalization(for: field))
                        .autocorrectionDisabled(field.inputKind == "email")
                }
            }
            fieldError(field)
        }
    ...
    private func multilineField(_ field: MobileClientCreationFormField) -> some View {
        VStack(alignment: .leading, spacing: LegendNextSpacing.tiny) {
            fieldLabel(field)
            TextEditor(text: valueBinding(for: field.key))
                .font(LegendNextTypography.body)
                .foregroundStyle(LegendNextColor.textPrimary)
    ...
    private func fieldLabel(_ field: MobileClientCreationFormField) -> some View {
    ...
    @ViewBuilder
    private func fieldError(_ field: MobileClientCreationFormField) -> some View {
        if let error = fieldErrors[field.key] {
            Text(error)
                .font(LegendNextTypography.caption)
                .foregroundStyle(LegendNextColor.danger)
                .fixedSize(horizontal: false, vertical: true)
        } else if let helpText = field.helpText, !helpText.isEmpty {
            Text(helpText)
                .font(LegendNextTypography.caption)
                .foregroundStyle(LegendNextColor.textSecondary)
        }
    ...
    private var submitTitle: String {
        guard let field = allFields.first(where: { $0.key == "RecordType" }) else {
            return "Create Client"
        }
        return "Create \(selectedLabel(for: field))"
    }

    private var allFields: [MobileClientCreationFormField] {
        form?.sections.flatMap(\.fields) ?? []
    }

    private var submissionErrorPresented: Binding<Bool> {
        Binding(
    ...
    private func loadForm() async {
        ...
        fieldErrors = [:]
        do {
            let loadedForm = try await store.clientCreationForm()
            form = loadedForm
            values = Dictionary(
                uniqueKeysWithValues: loadedForm.sections
                    .flatMap(\.fields)
    ...
    private func submit() async {
        ...
        do {
            let result = try await store.createClient(fields: submissionFields())
    ...
    private func localValidationErrors() -> [String: String] {
        var errors: [String: String] = [:]
        for section in form?.sections ?? [] where isVisible(section) {
            for field in section.fields where isVisible(field) && field.inputKind != "hidden" {
                if isRequired(field), values[field.key]?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
    ...
    private func submissionFields() -> [String: String] {
        var fields: [String: String] = [:]
        for section in form?.sections ?? [] where isVisible(section) {
            for field in section.fields where field.inputKind == "hidden" || isVisible(field) {
                let value = values[field.key]?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
                guard !value.isEmpty else { continue }
                fields[field.key] = field.inputKind == "date" ? "\(value)T00:00:00Z" : value
            }
        }
        return fields
    }
    ...
    private func applyValueRules() {
        for field in allFields {
            if let rule = field.valueRules.first(where: { conditionsMatch($0.conditions) }) {
                values[field.key] = rule.value
            }
        }
    }

    private func perform(_ action: MobileClientCreationFormAction) {
        guard let field = action.field, action.value == "today" else { return }
        updateValue(Self.dayFormatter.string(from: Date()), for: field)
    }

    private func isVisible(_ section: MobileClientCreationFormSection) -> Bool {
        conditionsMatch(section.visibleWhen)
    }

    private func isVisible(_ field: MobileClientCreationFormField) -> Bool {
        conditionsMatch(field.visibleWhen)
    }

    private func isRequired(_ field: MobileClientCreationFormField) -> Bool {
        field.required || conditionsMatch(field.requiredWhen)
    }

    private func conditionsMatch(_ conditions: [MobileClientCreationFieldCondition]) -> Bool {
        conditions.allSatisfy { condition in
            guard let currentValue = values[condition.field] else { return false }
            return condition.equalsAny.contains {
                $0.caseInsensitiveCompare(currentValue) == .orderedSame
            }
        }
    }

    private func selectedLabel(for field: MobileClientCreationFormField) -> String {
    ...
    private func dateLabel(for field: MobileClientCreationFormField) -> String {
        guard let value = values[field.key],
              let date = Self.dayFormatter.date(from: value) else {
            return field.placeholder ?? "Select date"
        }
        return date.formatted(date: .abbreviated, time: .omitted)
    }

    private func valueBinding(for key: String) -> Binding<String> {
        Binding(get: { values[key] ?? "" }, set: { updateValue($0, for: key) })
    }

    private func dateBinding(for key: String) -> Binding<Date> {
        Binding(
            get: { Self.dayFormatter.date(from: values[key] ?? "") ?? Date() },
            set: { updateValue(Self.dayFormatter.string(from: $0), for: key) })
    }

    private func keyboardType(for field: MobileClientCreationFormField) -> UIKeyboardType {
    ...
    private func contentType(for field: MobileClientCreationFormField) -> UITextContentType? {
    ...
    private func autocapitalization(for field: MobileClientCreationFormField) -> TextInputAutocapitalization {
        field.inputKind == "email" ? .never : .words
    }
...
private struct ClientCreationDateSelection: Identifiable {
    let id = UUID()
    let field: MobileClientCreationFormField
}

        ...
        const saveExpenseLensState = (extraState = {}) => {
            try {
                ...
                categoriesContainer.querySelectorAll(`[id^="${elId('CatRow')}"]`).forEach(row => {
                    ...
                    categories.push({ index, name, amount, due, frequency, paymentMethod, isTemplate, isPinned });
                });
                ...
                if (state) {
                    ...
                    if (state.categories && state.categories.length > 0) {
                        state.categories.forEach(cat => {
                            createCategoryRow(++categoryCount, cat.name, cat.amount, cat.due || '', cat.frequency || cat.recurrence, cat.paymentMethod || '', cat.isTemplate === true, cat.isPinned === true);
                            categoriesCreated++;
                        });
                    }
                }

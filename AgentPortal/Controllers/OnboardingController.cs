...
public class OnboardingController : Controller
{
    ...
    public OnboardingController(MasterAppDbContext db, IConfiguration config, ClientProvisioningService provisioning, ILogger<OnboardingController> logger, IEmailSender emailSender, PiiProtector pii)
    {
        _db = db;
        _config = config;
    ...
    /// <summary>
    /// Decrypts encrypted PII fields in-place on a submission loaded from the database.
    /// Called immediately after any DB load before passing to a view or PDF builder.
    /// </summary>
    private void DecryptSubmission(OnboardingSubmission s)
    {
        s.BankAccountNumber = _pii.Decrypt(s.BankAccountNumber, _logger, nameof(s.BankAccountNumber)) ?? "";
        s.BankRoutingNumber = _pii.Decrypt(s.BankRoutingNumber, _logger, nameof(s.BankRoutingNumber)) ?? "";
        s.DriverLicenseNumber = _pii.Decrypt(s.DriverLicenseNumber, _logger, nameof(s.DriverLicenseNumber));
        s.SsnLast4 = _pii.Decrypt(s.SsnLast4, _logger, nameof(s.SsnLast4));
    }
    ...
    public async Task<IActionResult> CreateInvite([Bind(Prefix = "NewInvite")] OnboardingInviteInputModel model)
    {
        ...
        // Block only when an active/pending invite already exists for this email.
        // Expired, Revoked, Submitted, or Archived invites do not block a fresh invite.
        var activeStatuses = new[] { "Pending", "Invited" };
        var activeExists = await _db.OnboardingInvites.AnyAsync(x =>
            x.NormalizedEmail == emailNorm &&
            activeStatuses.Contains(x.Status));
        if (activeExists)
        {
            ModelState.AddModelError(nameof(model.Email),
                "An active onboarding invite already exists for this email address. " +
                "Revoke or wait for it to expire before sending a new one.");
            var dashVm = await BuildDashboardAsync();
            dashVm.NewInvite = model;
            return View("Index", dashVm);
        }

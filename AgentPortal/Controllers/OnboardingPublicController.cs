...
public class OnboardingPublicController : Controller
{
    ...
    public OnboardingPublicController(MasterAppDbContext db, ILogger<OnboardingPublicController> logger, PiiProtector pii)
    {
        _db = db;
        _logger = logger;
        _pii = pii;
    ...
    public async Task<IActionResult> Start(OnboardingFormViewModel vm)
    {
        ...
        var submission = new OnboardingSubmission
        {
            ...
            SsnLast4 = _pii.Encrypt(vm.SsnLast4?.Trim()),
            SsnNote = vm.SsnNote?.Trim(),
            DriverLicenseNumber = _pii.Encrypt(vm.DriverLicenseNumber?.Trim()),
            ...
            BankRoutingNumber = _pii.Encrypt(vm.BankRoutingNumber.Trim())!,
            BankAccountNumber = _pii.Encrypt(vm.BankAccountNumber.Trim())!,

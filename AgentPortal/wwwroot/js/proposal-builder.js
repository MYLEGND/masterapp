(() => {
  ...
  async function openModal(id = null) {
    ...
    if (!record) {
      ...
      editingLeadId = (record.leadId || "").trim();
      editingLeadName = (record.leadName || "").trim();
  ...
  async function handleSave(e) {
    ...
    const leadIdForSave = (editingLeadId || meta.leadId || "").trim();
    const effectiveLeadName = (editingLeadName || meta.leadName || "").trim();

    const fallbackLeadId = leadIdForSave || meta.scopeKey || meta.pageTitle || "UNASSIGNED";
    const finalLeadId = fallbackLeadId.trim();
    ...
    try {
      ...
      alert(detail ? `Unable to save proposal right now: ${detail}` : "Unable to save proposal right now. Please try again.");
    } finally {
      isSaving = false;

...
function syncLeadProductionPill(row, paid, personal){
...
function setLeadProduction(row, status, amount, totals, personalAmount){
  ...
  syncLeadProductionPill(row, paid, personal);
...
function setLeadProductionById(leadId, status, amount, totals, personalAmount){
  const row = rows.find(r => r.dataset.clientId === leadId);
  if (row) setLeadProduction(row, status, amount, totals, personalAmount);
...
window.quickViewCalendarAdapter = {
  ...
  async applyResult(data, context){
    ...
    renderAppointmentSnapshot(
      activeClientDetail.latestAppointment || null
    );
...
async function boot(){
  ...
  loadMyDaySnapshot(true)
    .then(() => renderAll())
    .catch(error => {
      quickViewDiagnostics.error("Leads My Day snapshot failed during boot", error, "myday");
...
function startLeadsBoot(){
  void boot().catch(error => {
    quickViewDiagnostics.error("Leads CRM boot failed", error, "boot");

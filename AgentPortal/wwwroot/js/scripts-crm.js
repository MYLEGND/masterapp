(() => {
    ...
    document.addEventListener("click", async (e) => {
        ...
        if (callBtn) {
            ...
            try {
                ...
                if (res.ok){
                    ...
                    const next = json.callCount ?? json.attemptsLifetime ?? ((leads.find(l => l.leadId === id)?.callCount || 0) + 1);
                    updateCallCountEverywhere(id, next);
                    liveSync?.sendCall(id, next);
                } else {
                    console.error("IncrementCall failed", res.status);
                }
            } catch (err){
                console.error("IncrementCall error", err);
            }
        ...
        const res = await fetch(`/Leads/Lead?id=${encodeURIComponent(id)}`);
        if (!res.ok) return;

(() => {
  ...
  function initializeRealtime() {
    if (connection || !window.signalR?.HubConnectionBuilder) return;
  ...
  window.liveSync = {
    ...
    sendReorder(stageKey, orderedIds) {
      if (!stageKey || !Array.isArray(orderedIds) || !connection) return;
    ...
    sendUpdate(payload) {
      if (!payload || !payload.leadId || !connection) return;
  ...
  window.addEventListener('legend-signalr-ready', initializeRealtime);
  initializeRealtime();
})();

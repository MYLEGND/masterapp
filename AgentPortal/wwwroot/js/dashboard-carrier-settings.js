document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderCompRows(item) {
        const lines = (item.compensationLines || []).filter((_, idx) => idx > -1);
        if (!lines.length) {
            return `
    ...
    async function ensureLoaded(force = false) {
        ...
        store.loadingPromise = fetch("/Dashboard/CarrierSettings", {
            credentials: "include",
        })
            .then(async (response) => {
                if (!response.ok) {
    ...
    async function saveState() {
        ...
        try {
            const response = await fetch("/Dashboard/SaveCarrierSettings", {
                method: "POST",
                credentials: "include",
                headers: {
            ...
            setStatus("Could not save. Keep the window open and try another edit to retry.", "is-error");
        } finally {

(() => {
  ...
  function renderQuote(data) {
    ...
    if (meta2) {
      ...
      const definition = startMix
        ? `Entry Engaged = CTA click, first form interaction, completed discovery step, or contact-step progress. ${startMix}.`
        : 'Entry Engaged = CTA click, first form interaction, completed discovery step, or contact-step progress.';
      if (data.dropOffStartsToFormStarts != null && breakdown) {
        meta2.textContent = `Entry Engaged → Form Started drop-off: ${data.dropOffStartsToFormStarts}% · ${definition} ${breakdown}`;
      } else if (data.dropOffStartsToFormStarts != null && startMix) {
        meta2.textContent = `Entry Engaged → Form Started drop-off: ${data.dropOffStartsToFormStarts}% · ${definition}`;
  ...
  function renderFunnelContactFriction(abandon, funnel) {
    if (!abandon) return;

    const stageMetrics = Array.isArray(funnel?.stageMetrics) ? funnel.stageMetrics : [];
    const findStageCount = key => Number(stageMetrics.find(x => x.stageKey === key)?.count || 0);

    const contactAbandons = Number(abandon.contactStepAbandonCount || 0);
    const submitAttempts = findStageCount('submit_attempted');
    const contactViewed = findStageCount('contact_step_viewed');
    const denominator = contactViewed > 0 ? contactViewed : contactAbandons + submitAttempts;
    const rate = denominator > 0 ? (contactAbandons / denominator) * 100 : null;

    setText('quote-contact-abandon-count', contactAbandons);
    setText('quote-contact-submit-attempts', submitAttempts);
    setText('quote-contact-friction-rate',
      rate == null
        ? 'Contact-step abandon rate unavailable'
        : `Contact-step abandon rate: ${formatPct(rate)}`);

    const topFields = (abandon.topAbandonedFields || []).slice(0, 5);
    const topCompleted = (abandon.topLastCompletedFields || []).slice(0, 5);

    setText('quote-contact-top-field', topFields[0]?.fieldName || '—');

    renderTable('quote-contact-drop-fields-body', topFields, [
      { key: 'fieldName' },
      { key: 'quoteType' },
      { key: 'abandonCount', align: 'text-end' }
    ]);

    renderTable('quote-contact-last-completed-body', topCompleted, [
      { key: 'fieldName' },
      { key: 'quoteType' },
      { key: 'count', align: 'text-end' }
    ]);

    const note = document.getElementById('quote-contact-friction-note');
    if (note) {
      note.textContent = contactAbandons === 0 && topFields.length === 0
        ? 'No contact-step field drop-off recorded in this range yet.'
        : 'Contact-step drop-off is derived from contact-step progress and explicit form_abandon signals before confirmed lead submission.';
    }
  }
...
function escapeHtml(value) {
  ...
  async function loadQuote() {
    try {
      ...
      const abandon = results[1].status === 'fulfilled' ? results[1].value : null;
      if (data) renderQuote(data);
      if (abandon) {
        renderAbandonment(abandon);
        renderFunnelContactFriction(abandon, data);
      }
    } catch (err) {
  ...
  async function loadBehavior() {
    try {
      ...
      const time = results[1].status === 'fulfilled' ? results[1].value : {};
      const exit = results[2].status === 'fulfilled' ? results[2].value : {};
      const journey = results[3].status === 'fulfilled' ? results[3].value : {};
      const sources = results[4].status === 'fulfilled' ? results[4].value : {};
  ...
  async function loadConv() {
    try {
      ...
      if (t === 'all') {
        ...
        convData = results[0].status === 'fulfilled' ? results[0].value : null;
        summaryData = results[1].status === 'fulfilled' ? results[1].value : null;
      }
      if (!convData) return;
  ...
  async function loadTeamRollup() {
    try {
      ...
      const conv = results[3].status === 'fulfilled' ? results[3].value : null;
      const agentPerf = results[4].status === 'fulfilled' ? results[4].value : null;
      ...
      console.error('Team rollup failed', err);
    }
  }
// Modal wiring -----------------------------------------------------
  ...
  let deviceIntelligenceLoading = false;

  async function safeLoadDeviceIntelligence() {
    if (!window.websiteAnalyticsDeviceIntelligence || typeof window.websiteAnalyticsDeviceIntelligence.loadCurrentView !== 'function') {
      return;
    }

    if (deviceIntelligenceLoading) {
      return;
    }

    deviceIntelligenceLoading = true;

    try {
      await window.websiteAnalyticsDeviceIntelligence.loadCurrentView();
    } finally {
      deviceIntelligenceLoading = false;
    }
  }
...
;(() => {
  ...
  async function fetchCachedDeviceRequest(key, fetcher) {
    ...
    analyticsRequestCache.set(key, promise);
    return promise;
  }

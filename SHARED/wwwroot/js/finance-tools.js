...
const toast = typeof window.toast === "function" ? window.toast : (msg => console.log(msg || ""));
    ...
    const coachingToolLookup = new Map(
        coachingTools.map(tool => [tool.id, tool])
    );

    function syncCoachingToolSelectorState(toolId) {
        if (!coachingToolDropdown) return;

        const nextValue = String(toolId || "").trim();
        if (!nextValue) {
    ...
    const buildCoachingGraphicMarkup = (tool) => {
        ...
        switch (tool?.type) {
            ...
            case "term":
                return buildTermCoachingGraphic(tool);
            case "whole-life":
                return buildWholeLifeCoachingGraphic(tool);
            case "universal-life":
                return buildUniversalLifeCoachingGraphic(tool);
            default:
                return "";
        }
    };
    ...
    const requestCoachingToolSelection = (toolId) => {
        const nextToolId = String(toolId || "").trim();
        const tool = coachingToolLookup.get(nextToolId);
        if (!tool || !embedContainer) return;
    ...
    if (coachingToolDropdown && coachingToolSelectWrap && coachingTools.length > 0) {
        ...
        coachingToolDropdown.addEventListener("change", function () {
            ...
            this.blur();
            requestCoachingToolSelection(selectedToolId);
        });
...
if (t.id === "ExpenseLens" || t.id === "BusinessExpenseLens") {
    try {
        ...
        const EL_PAYMENT_FILTERS = [
            { value: EL_BILL_FILTER_DEFAULT_VALUE, label: '--Bills This Week--' },
            { value: 'all', label: 'All Bills' },
            { value: 'debit', label: 'Debit Bills' },
            { value: 'credit', label: 'Credit Bills' },
        ];
        ...
        const elPaymentFilterLabel = (value) => {
            ...
            return EL_PAYMENT_FILTERS.find(option => option.value === normalized)?.label || '--Bills This Week--';
        };

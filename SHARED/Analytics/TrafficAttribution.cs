    ...
    public static class TrafficAttribution
    {
            ...
            string? host = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(referrerHost))
            {
                ...
                if (ReferralSources.Any(s => referrerHost.Contains(s, StringComparison.OrdinalIgnoreCase)))
                    return TrafficType.Referral;

                if (referrerHost.Contains("chatgpt", StringComparison.OrdinalIgnoreCase) ||
                    referrerHost.Contains("openai", StringComparison.OrdinalIgnoreCase) ||
                    referrerHost.Contains("perplexity", StringComparison.OrdinalIgnoreCase) ||
                    referrerHost.Contains("claude", StringComparison.OrdinalIgnoreCase) ||
                    referrerHost.Contains("gemini", StringComparison.OrdinalIgnoreCase))
                {
                    return TrafficType.Organic;
                }
            ...
            // Modern browsers and privacy protections frequently strip referrer data.
            // If no attribution exists but the session is otherwise clean, treat as Direct
            // instead of Unknown so reporting better reflects real human traffic.

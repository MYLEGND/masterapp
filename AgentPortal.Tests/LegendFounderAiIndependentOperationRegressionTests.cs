...
public sealed class LegendFounderAiIndependentOperationRegressionTests
{
    ...
    public async Task SourceLanguageFailure_NativeOnlyStillFailsClosedWithZeroProviderCalls()
    {
        ...
        var operations = new Mock<ILegendConnectOperations>(MockBehavior.Strict);
        var handler = new RecordingProviderHandler();
        var detector = new FixedLanguageDetector(
    ...
    public void OwnedRecordClassification_FailsClosedAndNamesTheMissingRelation()
    {
        ...
        Assert.Equal(
            ...
            noAnalysis.Intent);
        Assert.True(noAnalysis.IsAnalysisUnavailable);
        Assert.Equal("owned_record_analysis_unavailable", noAnalysis.Diagnostic);
    ...
    public async Task TeacherMode_NoAdmittedRelation_DoesNotForceTheRegisteredReadTool()
    {
        ...
        operations.Verify(operation => operation.TryBindConversationContentAsync(
            "Give me the present record state you hold.",
        ...
        LegendConnectUtteranceMeaningGraphSnapshot graph)
    {
        ...
        operations
            .Setup(operation => operation.TryBindConversationContentAsync(
                It.IsAny<string>(),
                It.IsAny<LegendConnectDiscourseStateSnapshot?>(),
                It.IsAny<CancellationToken>(),
                It.IsAny<string>()))
    ...
    [Fact]
    public async Task ClientLeadPortfolio_UsesCanonicalRecordTypesAndCollapsesLinkedIdentities()
    {
        using var founderEnvironment = new FounderEnvironmentScope();
        using var writeSentinel = new WriteAttemptSentinel();
        await using var db = BuildSentinelDb(writeSentinel);
        var founder = await AddFounderProfileAsync(db);
        var canonicalIdentity = Guid.NewGuid().ToString();
        foreach (var userId in new[] { "legacy-client", "current-client" })
        {
            var profile = new ClientProfile
            {
                ClientUserId = userId,
                ExternalIdentityObjectId = canonicalIdentity,
                CrmStatus = "Active",
                // Canonical CRM normalization admits punctuation between words.
                CrmNotes = """{"recordType":"Business_Client"}"""
            };
        ...
        db.WorkstationLeadProfiles.Add(NewLead("open-lead", "Contacted"));
        db.WorkstationLeadProfiles.Add(NewLead("other-open-lead", "Sold"));
        db.WorkstationLeadProfiles.Add(NewLead("converted-lead", "Converted"));
        await db.SaveChangesAsync();
        writeSentinel.Arm();

        var snapshot = await BuildAgencyCommandService(db)
            .GetFounderPortfolioCountsAsync(founder);

        Assert.Equal(1, snapshot.ActiveClientCount);
        Assert.Equal(1, snapshot.AgentLinkedClientCount);
        Assert.Equal(2, snapshot.ActiveLeadCount);
        Assert.Equal(snapshot.ActiveLeadCount, snapshot.ActiveLeadsByCrmStatus.Sum(row => row.Count));
        Assert.Equal(0, writeSentinel.OperationalWriteAttempts);
        Assert.False(db.ChangeTracker.HasChanges());
    }

    [Theory]
    [InlineData("Lead", null, "Lead")]
    [InlineData("Lead", "Client", "Lead")]
    [InlineData(" lead ", "BusinessClient", "Lead")]
    [InlineData("Client", "BusinessClient", "Client")]
    [InlineData(null, null, "Client")]
    [InlineData(null, "BusinessClient", "BusinessClient")]
    public void CanonicalClientRecordType_PreservesExplicitTypeBeforePipelineAndGuidFallback(
        string? recordType,
        string? pipelineStage,
        string expected)
    {
        var clientUserId = Guid.NewGuid().ToString();
        var metadata = JsonSerializer.Serialize(new { recordType, pipelineStage });

        Assert.Equal(expected, ClientRecordClassification.Resolve(clientUserId, metadata));
        Assert.Equal(expected, ClientRecordClassification.Resolve(clientUserId, recordType, pipelineStage));
        Assert.Equal(expected == "Lead", ClientRecordClassification.IsLead(clientUserId, metadata, "Active"));
    }

    [Theory]
    [InlineData(null)]
    [InlineData("{}")]
    [InlineData("legacy non-JSON notes")]
    public void CanonicalClientRecordType_MissingMetadataPreservesLegacyIdentityFallback(string? metadata)
    {
        Assert.Equal("Client", ClientRecordClassification.Resolve(Guid.NewGuid().ToString(), metadata));
        Assert.Equal("Lead", ClientRecordClassification.Resolve("legacy-non-portal-contact", metadata));
        Assert.False(ClientRecordClassification.IsLead("legacy-non-portal-contact", metadata, "Active"));
    }

    [Theory]
    [InlineData("Lead")]
    [InlineData("Active")]
    public async Task ClientLeadPortfolio_ExplicitLeadWithGuidAndClientPipelineIsNotAnAgentLinkedClient(string crmStatus)
    {
        using var founderEnvironment = new FounderEnvironmentScope();
        using var writeSentinel = new WriteAttemptSentinel();
        await using var db = BuildSentinelDb(writeSentinel);
        ...
        var profile = new ClientProfile
        {
            ...
            CrmStatus = crmStatus,
            CrmNotes = """{"recordType":"Lead","pipelineStage":"Client"}"""
        };
        db.ClientProfiles.Add(profile);
        db.ClientEntitlements.Add(new ClientEntitlement
        {
        ...
        var snapshot = await BuildAgencyCommandService(db).GetFounderPortfolioCountsAsync(founder);
    ...
    [Fact]
    public async Task NativeReadOnlyBinding_RequiresFounderBeforeReadingRemediationStatus()
    {
        using var founderEnvironment = new FounderEnvironmentScope();
        await using var db = ControllerTestHelpers.BuildDb();
        var operations = new Mock<ILegendConnectOperations>(MockBehavior.Strict);
        ...
        await Assert.ThrowsAsync<ForbidResultException>(() =>
            authority.BindReadOnlyResultAsync(
                ControllerTestHelpers.BuildUser("not-the-founder"),
                StatusBindingRequest(),
                CancellationToken.None));

        Assert.Empty(remediation.Invocations);
        Assert.Empty(operations.Invocations);
    }

    [Fact]
    public async Task NativeReadOnlyBinding_CancellationCannotMintAReceipt()
    {
        using var founderEnvironment = new FounderEnvironmentScope();
        await using var db = ControllerTestHelpers.BuildDb();
        ...
        var operations = new Mock<ILegendConnectOperations>(MockBehavior.Strict);
        var remediation = new Mock<IFounderSoftwareRemediationService>(MockBehavior.Strict);
        remediation.Setup(service => service.GetStatusAsync(cancellation.Token))
            .Callback(() => cancellation.Cancel())
            .ReturnsAsync((object)new { available = true });
        var authority = new LegendFounderToolAuthority(
            new FounderLegendConnectService(operations.Object, new AgentProfileAccessResolver(db)),
            remediation.Object);
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            authority.BindReadOnlyResultAsync(
                ControllerTestHelpers.BuildUser(FounderEnvironmentScope.FounderId),
                StatusBindingRequest(),
                cancellation.Token));

        remediation.Verify(service => service.GetStatusAsync(cancellation.Token), Times.Once);
        Assert.Empty(operations.Invocations);
    }

    [Fact]
    public async Task CanceledToolDispatch_DoesNotInvokeTheRegisteredExecutor()
    {
        await using var db = ControllerTestHelpers.BuildDb();
        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            authority.ExecuteAsync(
                ControllerTestHelpers.BuildUser(),
                new FounderAiToolCall("canceled-read", "legend_software_remediation_status", "{}"),
                "legend",
                cancellation.Token));
    ...
    [Fact]
    public void NativeReadOnlyReceipt_PreservesExactBindingAndRejectsFutureObservation()
    {
        var now = DateTime.UtcNow;
        var request = StatusBindingRequest() with { ObservedUtcPath = "observedUtc" };
        var output = JsonSerializer.Serialize(new { available = true, observedUtc = now });
        ...
        Assert.Equal("read_only_content_binding_receipt_governed", reason);
        Assert.Equal(request.RequestIdentity, receipt.RequestIdentity);
        Assert.Equal(request.TransitionSignature, receipt.TransitionSignature);
        Assert.Equal(request.ResultSemanticFrameSignature, receipt.ResultSemanticFrameSignature);
        Assert.Equal(request.ToolName, receipt.ToolName);
        Assert.Equal(LegendLanguageIdentity.TextHash(request.ArgumentsJson), receipt.ArgumentsHash);
        Assert.Equal(request.ValuePath, receipt.ValuePath);
        Assert.Equal(request.SemanticVariable, receipt.SemanticVariable);
        Assert.Equal(request.ResultDimension, receipt.ResultDimension);
        Assert.Equal("true", receipt.SemanticValue);
        Assert.Equal(LegendLanguageIdentity.TextHash(output), receipt.OutputHash);
        Assert.Equal(now, receipt.ObservedUtc);
        Assert.Equal(now, receipt.ExecutedUtc);
        ...
        Assert.False(LegendFounderToolAuthority.TryCreateReadOnlyContentBindingReceipt(
            request,
            JsonSerializer.Serialize(new { available = true, observedUtc = now.AddSeconds(1) }),
            now,
            out var futureReceipt,
            out var futureReason));
        Assert.Null(futureReceipt);
        Assert.Equal("read_only_content_binding_output_malformed", futureReason);
    }

    private static LegendConnectReadOnlyContentBindingRequest StatusBindingRequest() =>
        new(
            "status-binding-request",
            "status-transition-signature",
    ...
    public void NativeDiagnosticReceipt_RejectsFailedStageZeroButRetainsIndependentAvailableStage(string state)
    {
        var output = JsonSerializer.Serialize(new
        {
            providerCapacity = new { monthlyCharactersConsumed = 0 },
            runtimePolicy = new { learningEnabled = true },
            stages = new[]
            {
                new { name = "provider_capacity", state },
                new { name = "runtime_policy", state = "available" }
            }
        });
        var request = StatusBindingRequest() with
        {
            ToolName = "legend_operational_diagnostics",
            ValuePath = "providerCapacity.monthlyCharactersConsumed"
        };
        Assert.False(LegendFounderToolAuthority.TryCreateReadOnlyContentBindingReceipt(
            request, output, DateTime.UtcNow, out var failed, out var reason));
        Assert.Null(failed);
        Assert.Equal("read_only_content_binding_source_unavailable", reason);

        Assert.True(LegendFounderToolAuthority.TryCreateReadOnlyContentBindingReceipt(
            request with { ValuePath = "runtimePolicy.learningEnabled" },
            output, DateTime.UtcNow, out var independent, out _));
        Assert.Equal("true", independent!.SemanticValue);
    }
        ...
        TranslationDetectionResult result) : ITranslationService
    {
            ...
            LegendConnectExternalProviderPolicy? providerPolicy)
        {
            ...
            ObservedPolicy = LegendConnectExternalProviderPolicy.Resolve(providerPolicy);
            DetectionCount++;
            return Task.FromResult(result);
        }

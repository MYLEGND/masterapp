...
public sealed class LegendFounderAiNativeOnlyProviderIsolationTests
{
    ...
    private static TestScope BuildProductionEquivalentScope(
        ...
        SaveChangesInterceptor? writeSentinel = null,
    ...
    // Reuse the complete production registration graph for capability proofs.
    // Only external transport and the in-memory database are test boundaries;
    // curriculum admission, meaning composition, planning and realization stay real.
    internal static async Task WithProductionAuthorityAsync(
        Func<IServiceProvider, MasterAppDbContext, Func<(int Clients, int Sends)>, Task> verify,
        SaveChangesInterceptor? writeSentinel = null)
    {
        await using var scope = BuildProductionEquivalentScope(writeSentinel: writeSentinel);
        await verify(
            scope.Scope.ServiceProvider,
            scope.Db,
            () => (scope.External.CreatedClients.Count, scope.External.SendAttempts));
    }
    ...
    /// bypass. An explicit source request still requires research. The same
    /// question with unknown native meaning cannot acquire public-fact
    /// authority merely from its wording. Neither decision contacts a provider.
    /// </summary>
    [Theory]
    [InlineData("Who currently holds the record for the deepest ocean dive?", false)]
    [InlineData("Who currently holds the record for the deepest ocean dive?", true)]
    [InlineData("What is the current published exchange rate for the euro?", false)]
    [InlineData("What is the current published exchange rate for the euro?", true)]
    public async Task NativeOnly_ExplicitSourceRequestRequiresResearchButUnknownQuestionDoesNotInventPublicAuthority(
        string publicFactRequest, bool explicitlyRequestsSources)
    {
        using var founderEnvironment = new FounderEnvironmentScope(FounderId);
        await using var scope = BuildProductionEquivalentScope();
        ...
        var operations = scope.Resolve<ILegendConnectOperations>();
        var inference = await operations.TryInferConversationWithDiscourseAsync(
            explicitlyRequestsSources ? publicFactRequest + " Please cite sources." : publicFactRequest,
            Array.Empty<LegendConnectConversationContextItem>(),
            new LegendConnectDiscourseStateSnapshot([]),
            CancellationToken.None,
            "en",
        ...
        Assert.Equal(explicitlyRequestsSources, decision.ResearchRequired);
        Assert.Equal(

#!/usr/bin/env python3
"""Validate an explicitly authorized known-failure baseline; never rewrite TRX outcomes.
...
and selection of the current run's TRX are the caller's release authority.
"""
import argparse
...
def validate(trx_path, manifest_path, source_root, runner_exit_code, run_start_utc, production_base_sha):
    ...
    require(results and Counter(r.get('testName') for r in results) == Counter(names), 'Full result roster differs from approved baseline')
    ...
    return {'status': 'AcceptedKnownBaseline' if failures else 'AcceptedImprovedBaseline',
            'allTestsPassed': False, 'failedOutcomesRetained': True,
            'total': len(results), 'passed': counts['Passed'], 'failed': counts['Failed'],
            'notExecuted': counts['NotExecuted'], 'knownFailures': failures,
            'skippedTests': sorted(skipped_names), 'rosterSha256': roster_digest(names),

...
namespace Protect_Website.Controllers
{
    ...
    public class CommercialQuoteController : Controller
    {
        ...
        public CommercialQuoteController(IConfiguration configuration, AgentTrackingResolver resolver,
            MasterAppDbContext db, IMetaConversionsApiService metaConversionsApi, IMetaPixelResolutionService metaPixelResolution, IMetaSignalIntelligenceService metaSignalIntelligence, IWebsiteLifeLeadCaptureService websiteLeadCapture, IProtectEmailSender emailSender, ILogger<CommercialQuoteController> logger)
        {
        ...
        public async Task<IActionResult> Commercial(CommercialQuoteFormModel model)
        {
            ...
            try
            {
                ...
                await _metaSignalIntelligence.RecordConfirmedLeadAsync(
                    new MetaSignalConfirmedLeadRequest
                    {
                        ...
                        QuoteType = "commercial",
                        PageKey = "quote_commercial",
                        EffectivePageKey = "quote_commercial",
                        PageVariant = "website",
                        PageMode = "site_mode",
            ...
            catch (Exception signalEx)
            {
                _logger.LogError(signalEx,
                    "CommercialQuote [{CorrelationId}]: meta signal lead recording failed for lead {LeadId} — lead is saved, continuing",
                    correlationId, lead.LeadId);
            }

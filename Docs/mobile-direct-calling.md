...
## UTC call wire contract

SQL `datetime2` retains the UTC clock value but reloads it with an unspecified `DateTime.Kind`. The shared call snapshot restores UTC for creation, expiry and receipt before status, signaling or push serialization. Every native client receives an explicit timezone; platform parsers remain strict. Verification includes a relational database reload because a newly created invite alone retains its original UTC kind and cannot expose this defect. Existing call-state and failure messages are collected into the same retained application-copy manifest as other interface text.

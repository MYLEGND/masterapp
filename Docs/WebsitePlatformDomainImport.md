...
LEGEND's `WebsiteDomainBinding` is the tenant/domain ownership authority. Cloudflare custom metadata is not required for new bindings. A legacy Cloudflare hostname can be recovered only when its retained LEGEND metadata matches the exact local binding/business pair; an unrelated provider hostname is rejected rather than adopted. A hostname is active only when Cloudflare confirms hostname ownership and an active certificate, and an HTTPS request to `/.well-known/legend-website` returns the exact binding and business IDs. The probe uses the existing public-network socket policy, disallows redirects, and uses normal TLS certificate verification.

Customer-domain traffic enters the SaaS zone through one wildcard Cloudflare Worker route. The Worker is transport only: it does not store tenant/domain mappings or decide ownership. LEGEND-owned `mylegnd.com` hosts pass through unchanged. External customer hostnames are forwarded to the existing Protect Website origin at `masterapp-protect.azurewebsites.net`, with the original hostname carried in an authenticated internal header. Protect accepts that header only when a shared bridge secret matches and the request reached its configured Azure origin; otherwise `Request.Host` remains authoritative. The shared secret is generated/reused by the approved release lane, stored in Protect App Service configuration and as a Cloudflare Worker secret, and is never sent to browsers or logs. This avoids per-customer Azure custom-domain bindings while preserving the existing business-domain database as the only routing authority.

The management response exposes provider verification/certificate state and the configured CNAME target. It never alters a customer's registrar or email DNS. Apex domains require registrar-supported alias/flattening or provider apex support; do not promise unsupported apex setup. Apex and www are separate verified bindings. Provider errors retain pending state. Missing configuration fails explicitly.
...
Before release, verify database mapping/migration, domain uniqueness and concurrency, revoked membership rejection, draft/public isolation, media authorization, hostile archive/URL rejection, imported route rendering, real provider verification failures, successful HTTPS routing, and export/reimport against the exact staged SHA. A website-router release must additionally pass the Worker unit/dry-run checks, prove LEGEND-owned hosts still bypass the bridge, deploy only with the shared bridge authority present, and complete the exact custom-host `/.well-known/legend-website` canary before the release is accepted.

...
await budget.settle(context, {
  reservationId, actualCostMicrousd, usageKnown, executionCompleted // optional; verified tools only
}); // { chargedMicrousd, usageKnown, overReservation }
```

Reservations are one-shot; repeated reservation IDs are denied rather than granting a second dispatch. The full worst-case cost is charged before work. Known actual cost returns only the unused amount and releases its concurrency lease. Unknown cost remains permanently charged at the reservation. Unconfirmed execution retains its lease until the **request** deadline, including across object restart, cancellation and request close. A broker-verified, authenticated completed tool receipt may explicitly carry `known:false`, `costEvidence:reserved_upper_bound`, and the exact full operator reservation. Only then does the trusted broker set `executionCompleted:true`: the full debit remains immutable while the completed execution lease is released. Model timeouts and ambiguous callbacks cannot use that completion claim. A later conflicting settlement cannot refund an unknown debit. Repeating the same settlement is idempotent. Observed overspend is recorded, never clipped to the reservation, and causes a typed failure with cost evidence.
...
`LegendFounderCloudActionApprovalTests.cs` contains 40 focused cases, including real SQLite concurrent claims/read admission/unique constraints, fresh-authority durable replay for reads and mutations, uncertain execution, exact argument/scope changes, revoked profile or terminal operation, legacy/model confirmation denial, isolated persistence, Worker digest interoperability, and rejection of numeric precision loss during canonicalization. The lead's integrated focused .NET run passed **111/111, zero skipped**, including these 40 cases, after the shared Worker fixture was explicitly copied into the repository's relocated test output. The only earlier failure was missing fixture discovery, not a digest mismatch; all four frozen cross-language vectors now pass unchanged. The integrated candidate is `69cff279`. These are local tests with synthetic service responses, not live callback or cloud execution qualification. The previously reported Worker tests remain a separate verification result.
...
Final receipt boundaries: callback wrappers are limited to 65536 bytes and their serialized output separately to 32768 UTF-8 bytes. Oversized or malformed receipts do not establish completion or authorize a refund. Cloud portfolio output projects numeric totals only; editable CRM grouping labels and freeform definitions are excluded. Cloud capability metadata reports the same currently exposed read catalog, not unavailable mutations.

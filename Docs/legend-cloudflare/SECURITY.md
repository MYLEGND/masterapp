...
`LegendFounderCloudActionApprovalTests.cs` contains 35 focused cases, including real SQLite concurrent claims/unique constraints, fresh-authority durable replay, uncertain execution, exact argument/scope changes, revoked profile or terminal operation, legacy/model confirmation denial, isolated persistence, and Worker digest interoperability. At this commit these .NET cases are authored but **not run**; a .NET build slot is coordinated by the lead. The previously reported Worker tests remain a separate verification result.

...
After lead integration run `node --test Legend-Cloudflare/tests/security/application.integration.mjs`; `LEGEND_INTEGRATION_ROOT` optionally identifies the exact other checkout to review. Application tests require the integrated entrypoint/security/runtime files and remain pending until that integration exists.

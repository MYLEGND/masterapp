...
A minimal reuse path is a bounded typed v2 extension of the **existing canonical turn receipt**, owned by `MessagingService`, containing per-action approval/execution entries. Bind each entry to user, session, operation, digest, environment and expiry; use the existing message row concurrency mechanism to transition `Approved → Executing` before dispatch, then persist `Completed` or `OutcomeUnknown`. Check the current object ACL immediately before each effect. Returning an existing completed receipt is allowed; an uncertain receipt never authorizes a repeated effect. Preserve the repair batch's stronger per-repository operation fence as well. If the canonical owner instead chooses a dedicated execution-receipt entity in the same Azure context, it must remain subordinate to the existing authority and cannot become a copied role/permission store.

Do not append arbitrary approval properties or service-message rows without changing the canonical contract: the current turn JSON uses `UnmappedMemberHandling.Disallow`, accepts only receipt version 1, and treats a service response linked to a user turn as terminal. A careless metadata insertion could invalidate history or prematurely complete the operation. The approval UI must display the exact proposed action before creating the digest-bound approval; a broad `FounderCommandConfirmed` value sent before model-selected arguments exist cannot satisfy this requirement.

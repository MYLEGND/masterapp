...
- Azure SKU synchronization is not Azure usage synchronization. Production `masterapp-translator-1221` is F0 (read-only Azure inspection). Actual production F0 metric definitions include TextCharactersTranslated. Direct Azure Monitor REST returned 1,932,766 month-to-date characters on 2026-09-11 local time; 67,234 remain against its 2,000,000 allowance at that observation. All recognized tiers are queried for observations. Azure telemetry can lag, so the application ledger and pending reservations remain distinct; no exact instantaneous billing balance is claimed.
- Remaining safe capacity now deducts its reserved live allowance. Durable reservations and completed usage are not erased by a stale request timestamp or future clock skew. Provider observation timestamps describe retrieval, not complete billing coverage. The earlier F0 no-metrics assumption from a Q&A source was disproven by this live resource query and removed.
...
Live F0 correction also affects enforcement: the existing capacity authority protects against observed Azure consumption plus local completions after the query anchor and in-flight reservations, without summing the entire Azure and local ledgers. Completing a reservation cannot refill availability while Azure telemetry remains cached. Training respects the live reserve. Telemetry lag before the anchor and usage outside LEGEND remain explicitly unverified; no exact instantaneous provider balance is promised.

## Final recorded checks

Combined backend candidate 40aec2d2: `/private/tmp/legend-connect-f0-final-results/full.trx`, 2,501 tests; 2,488 passed, nine failed, four skipped. All nine failure names and exact failure-message hashes match the prior call-experience baseline; the same four cases remain skipped, with no new failures. Comparison artifact: `/private/tmp/legend-connect-f0-final-results/baseline-comparison.json`. These are inherited failures, not a fully passing AI evaluation.

Native artifacts: Android debug assemble succeeded (`/private/tmp/legend-connect-android-package.log`); signed generic-device iPhone Debug build succeeded (`/private/tmp/legend-connect-ios-package.log`, app under `/private/tmp/legend-connect-ios-device/Build/Products/Debug-iphoneos/Legend.app`). These are test builds, not uploaded store releases. Native policy tests: Android one adaptive test and iOS seven direct-calling tests passed.

Final static cleanup after the backend run removed the old standalone CSS/JS (they existed under SHARED; the initial missing-assets assumption was incorrect), retained custom-input required validation within Connect, and passed JS syntax plus `/private/tmp/connect-ui-race-tests.cjs` and `/private/tmp/connect-ui-observed-tests.cjs`. The existing application-localization MutationObserver covers inserted modal content; the shared copy manifest contains 5,243 entries.

No production deployment, relay purchase/provisioning, role change, App Store upload or Google Play upload occurred. Main checkout remains preserved. Required browser visual validation could not run because supported browser discovery returned no browser; a connection request remains pending. Physical weak-network/forced-relay validation is not executed. Existing-VM start/stop management is implemented, but first-time in-modal purchasing/provisioning is not implemented or verified. A resource and narrow Azure management access are prerequisites, not proof of that missing purchasing flow.

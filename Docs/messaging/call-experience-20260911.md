...
Emulator investigation: Pixel_10_Pro had hw.camera.front=emulated, a synthetic input. The Mac exposes webcam0 as FaceTime HD Camera. Backed up config to /private/tmp/legend-emulator-config-before-call-demo.ini, changed front camera to webcam0 and cold-booted with data retained. Kept original 2 GB RAM and automatic GPU selection after 4 GB worsened host memory pressure. Emulator still reports software rendering due host memory pressure; performance is not yet validated. The synthetic source plausibly explains colored frames but a real-webcam call is required to close that observation.

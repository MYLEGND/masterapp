...
- **Business contact identity matches Protect:** public business forms use exact field names `FirstName`, `LastName`, `Phone`, and `Email`, plus `Message`. They persist into the existing `WebsiteLead` fields. CRM capture, analytics, the existing Meta server dispatcher, inquiry notifications, and the authorized inquiry-management projection consume that same lead; no business-only Meta sender, duplicate phone column, or duplicate lead record is introduced.

...
public static class LegendConnectOwnedRecordRequest
{
    ...
    /// run. It is never collapsed into "no intent": the caller must stop before
    /// an answer is served and surface the diagnostic.
    /// </summary>
    ...
    /// reported as unavailable analysis rather than guessed.
    /// </summary>
        ...
        LegendConnectUtteranceMeaningGraphSnapshot? graph)
    {
        if (graph is null)
        {
            return AnalysisUnavailable("owned_record_analysis_unavailable");
        }

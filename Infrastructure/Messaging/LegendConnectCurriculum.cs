...
internal sealed class LegendConnectCurriculumService : ILegendConnectStructuralCompositionGate
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        var candidates = await (
            ...
            where node.LanguageCode == languageCode && node.SupersededUtc == null &&
                ...
                primitive.ContradictionCount == 0 && primitive.IndependentSourceCount >= 1
        ...
        // Rank semantic interpretations at each controlled surface slot. A
        // production-mature meaning always suppresses lower-maturity meanings
        // for that same dimension and span. If no mature meaning exists, keep
        // only the strongest Founder-observed support level; equal-strength
        // competing values remain present so ambiguity still fails closed.
        nodes = nodes
            .GroupBy(item => new
            {
                item.StartTokenIndex,
            ...
            {
                var mature = group
                    .Where(item => item.IndependentSupportCount >= 3)
                    .ToArray();
                if (mature.Length > 0)
                    return mature;
                var strongest = group.Max(item => item.IndependentSupportCount);
                return group
                    .Where(item => item.IndependentSupportCount == strongest)
                    .ToArray();
            })
            .ToList();
        ...
        if (nodes.Count > 1)
        {
            ...
            var preferredRelations = learnedRelations
                ...
                {
                    var mature = group
                        .Where(item => item.MaturityState == "Supported" &&
                            item.IndependentSourceCount >= 3)
                        .ToArray();
                    if (mature.Length > 0)
                        return mature;
                    var strongest = group.Max(item => item.IndependentSourceCount);
                    return group
                        .Where(item => item.IndependentSourceCount == strongest)
                        .ToArray();
                })
                .ToArray();
            foreach (var relation in preferredRelations)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new LegendSemanticTransitionInference(
            ...
            [
                selected.EvidenceTier == LegendSemanticTransitionEvidenceTier.ProductionEligible
                    ? "independently_supported_semantic_transition"
                    : "founder_observed_semantic_transition",
                "canonical_endpoint_realization"
            ]);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new LegendSemanticTransitionInference(
            ...
            [
                ...
                selection.Selected.EvidenceTier == LegendSemanticTransitionEvidenceTier.ProductionEligible
                    ? "independently_supported_semantic_transition"
                    : "founder_observed_semantic_transition",
                "governed_content_binding",
                realization.LayoutEvidenceCount >= 3
                    ? "independently_supported_original_compositional_realization"
                    : "founder_observed_original_compositional_realization"
            ]);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return selection.Plan is null
            ...
            : new(true, selection.ReasonCode, selection.Plan);
    }
        ...
        LegendConnectUtteranceMeaningGraphSnapshot? composedGraph, CancellationToken cancellationToken)
    {
        ...
        if (candidates.Count == 0)
        {
            ...
            // Preserve the production tier as the default authority. Only
            // when no production-eligible response survives may runtime use
            // a contradiction-free Founder-observed transition. This is the
            // same transition store and selector—not a fallback responder—and
            // the retained evidence count remains below the production tier.
            // Conflicting strongest observations remain ambiguous.
            if (candidates.Count == 0)
            {
                if (HasContradictedSemanticTransition(
                        responseObservations,
                ...
                {
                    return SemanticTransitionSelection.Contradicted(
                        "semantic_transition_founder_observed_contradicted",
                        language,
                        sourceComponents);
                }

                var observedCandidates = BuildGovernedSemanticTransitionCandidates(
                        responseObservations,
                        values,
                        allowMissingVariables: false,
                        minimumEvidenceTier: LegendSemanticTransitionEvidenceTier.FounderObserved)
                    .ToList();
                if (observedCandidates.Count == 0)
                {
                    observedCandidates = BuildGovernedSemanticTransitionCandidates(
                            responseObservations,
                            values,
                            allowMissingVariables: false,
                            allowMissingStaticDimensions: true,
                ...
                if (observedCandidates.Count == 0)
                {
                    var observedPartialCandidates = BuildGovernedSemanticTransitionCandidates(
                            responseObservations,
                            values,
                            allowMissingVariables: true,
                            allowMissingStaticDimensions: true,
                        ...
                        .Where(item => item.MissingVariables.Count > 0 && item.DirectSourceMatchCount > 0)
                        .ToList();
                    if (observedPartialCandidates.Count > 0)
                    {
                        observedCandidates = (await BindCandidatesFromCurrentSourceEndpointAsync(
                                language,
                                input,
                                observedPartialCandidates,
                                cancellationToken))
                            .ToList();
                        if (observedCandidates.Count == 0)
                        {
                            observedCandidates = (await BindCandidatesFromCurrentLexicalVariableEvidenceAsync(
                                    language,
                                    input,
                                    observedPartialCandidates,
                                    cancellationToken))
                        ...
                        if (observedCandidates.Count == 0)
                        {
                            ...
                            contextFrames.AddRange(await ResolveGroundedContextFramesAsync(
                                ...
                                responseObservations,
                                cancellationToken));
                            observedCandidates = BindCandidatesFromGroundedContext(
                                    observedPartialCandidates,
                                    contextFrames)
                                .ToList();
                        }
                ...
                if (observedCandidates.Count > 0)
                {
                    ...
                    {
                        return SemanticTransitionSelection.Ambiguous(
                            "ambiguous_founder_observed_semantic_transition",
                            language,
                            sourceComponents);
                    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return observations
            ...
            .Where(group => TryGetGovernedSemanticTransitionFrames(
                group,
                LegendSemanticTransitionEvidenceTier.ProductionEligible,
                out _,
                out _,
                out _,
                out _))
    ...
    private static IReadOnlyList<SemanticTransitionCandidate>
        BuildGovernedSemanticTransitionCandidates(
            ...
            LegendSemanticTransitionEvidenceTier minimumEvidenceTier =
                LegendSemanticTransitionEvidenceTier.ProductionEligible)
    {
    ...
    private static bool TryGetGovernedSemanticTransitionFrames(
        ...
        out LegendSemanticTransitionEvidenceTier evidenceTier)
    {
        var group = observations as IReadOnlyList<SemanticTransitionObservation> ?? observations.ToList();
        independentEvidenceCount = 0;
        sourceFrame = null!;
        resultFrame = null!;
        evidenceTier = LegendSemanticTransitionEvidenceTier.None;
        var assessment = LegendSemanticTransitionProductionEligibility.Assess(group.Select(item =>
                new LegendSemanticTransitionEligibilityObservation(
                    ...
                    item.IsHumanVerifiedSupport)));
        if (assessment.Tier < minimumEvidenceTier)
        {
            return false;
        }

        independentEvidenceCount = assessment.IndependentSourceCount;
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Semantic selection remains tied to the selected transition's exact
        // Founder-approved endpoints. Articulation may reuse every active
        // Founder example with the same result frame so language learned in
        // one family can make already-selected meaning expressible in another.
        // This cross-use supplies layout only; it cannot change result meaning,
        // bind a fact, or introduce an unrelated response endpoint.
        var layoutExamples = examples;
        ...
        var layoutGroups = layouts
            ...
            .ToList();
        var matureLayoutGroups = layoutGroups
            .Where(group => group.IndependentFamilies >= 3)
            .ToList();
        var strongestObservedLayoutCount = layoutGroups.Count == 0
            ? 0
            : layoutGroups.Max(group => group.IndependentFamilies);
        var eligibleLayouts = matureLayoutGroups.Count > 0
            ? matureLayoutGroups
            : layoutGroups
                .Where(group => group.IndependentFamilies == strongestObservedLayoutCount)
                .ToList();
        if (scopedExamples.Count > 0)
        {
            ...
            if (hasResultVariables &&
                TryRealizeGovernedCrossArticulation(
                    ...
                    eligibleLayouts.Max(item => item.IndependentFamilies),
    ...
    /// forms of the same already-selected semantic result. Independently
    /// supported forms are always preferred; otherwise the strongest available
    /// contradiction-free Founder-observed shape may be used. The current
    /// bound value must already have an exact scoped endpoint for that form.
    /// No words are spliced between forms; every sentence remains observed
    /// Founder language, while the multi-form response itself is newly composed.
    /// </summary>
    private static bool TryRealizeGovernedCrossArticulation(
        IReadOnlyList<SemanticRealizationLayout> governedLayouts,
        ...
        out int evidenceCount)
    {
        text = string.Empty;
        evidenceCount = 0;
        if (governedLayouts.Count == 0 || scopedEndpoints.Count == 0 ||
            minimumIndependentFamilies <= 0)
            return false;
        ...
        var forms = governedLayouts
            ...
            .Where(group => group.IndependentFamilies >= minimumIndependentFamilies)
        ...
        return evidenceCount >= minimumIndependentFamilies;
    }

...
- For direct support, the claim statement and supporting_excerpt must be character-for-character identical copies of one short exact factual span from the candidate source; never join sentences, normalize punctuation, paraphrase either field, or add context that the source span does not state.
